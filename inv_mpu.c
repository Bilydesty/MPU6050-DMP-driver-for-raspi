...
{
	u8 res=0;
	if ((fd = wiringPiI2CSetup(SlaveAddress) )< 0) //初始化 IIC 总线
	{
		printf("wiringPi2CSetup 失败\n");
	}
...
#define LOAD_CHUNK  (16)
	...
	{
		printf("ERR:DMP should only be loaded once\n");
		return -1;
	}
